...
for idx, (fn, img_file) in tqdm(enumerate(img_files)): 
    output_img = os.path.join(args.output_dir, fn)
    
    img = dlib.load_rgb_image(img_file)
    dets = detector(img, 1)
    if len(dets) <= 0:
        print("no face landmark detected at frame", idx, "adding dummy box and face")
        bounding_boxes.append([-1,-1,-1,-1,-1,-1,-1,-1])
    ...
    bounding_boxes.append(quad.flatten())
