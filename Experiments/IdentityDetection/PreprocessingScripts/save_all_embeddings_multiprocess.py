...
def process_frame(frames_path, output_path, i):
...
for category, idx in zip((['real']*7)+(['fake']*3), list(range(0, 7)) + list(range(1, 6, 2))):
    ...
    func = partial(process_frame, frames_path, output_path)
    process_map(func, list(range(1, num_frames+1)), max_workers=os.cpu_count()-2)
