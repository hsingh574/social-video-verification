...
def parse_args():
    ...
    parser.add_argument('--results-dir', type=str, default='Robust PCA method Results',
                    help='Directory where results have been saved')
    parser.add_argument('--thresholds-main', nargs="+", default=[1.3, 1.5, 1.7, 1.9, 2.1])
    parser.add_argument('--threshold-idx-main', type=int, default=0)

    parser.add_argument('--results-dir-base1', type=str, default='PCA method Results',
                    help='Directory where baseline 1 results have been saved')
    parser.add_argument('--thresholds-base1', nargs="+", default=[1.05, 1.1, 1.15, 1.2, 1.3])
    parser.add_argument('--threshold-idx-base1', type=int, default=2)

    parser.add_argument('--results-dir-base2', type=str, default='L2 method Results',
                    help='Directory where baseline 2 results have been saved')
    parser.add_argument('--thresholds-base2', nargs="+", default=[1.3, 1.5, 1.7, 1.9, 2.1])
    parser.add_argument('--threshold-idx-base2', type=int, default=4)
...
def plot_vs_baselines_ROC(ids, threshes, window_size, results, save_dir):
    ...
    for method in results:
        if method == "Ours":
            fpr1, tpr1, fpr2, tpr2, fpr3, tpr3 = plot_ROC(ids, threshes[method], window_size, results[method], save_dir)
        else:
            fpr1, tpr1, fpr2, tpr2, fpr3, tpr3 = plot_ROC_v2(ids, threshes[method], window_size, results[method], save_dir)
...
def main():
    ...
    window_size = 350
    thresholds = {'Ours':args.thresholds_main, 'PCA': args.thresholds_base1, 'L2': args.thresholds_base2}
    threshold_idx = {'Ours':args.threshold_idx_main, 'PCA': args.threshold_idx_base1, 'L2': args.threshold_idx_base2}

    # Change dictionary entry titles depending on your baselines/baseline order input
    results = {'Ours':args.results_dir, 'PCA': args.results_dir_base1, 'L2': args.results_dir_base2}
    thresholds
