...
def noPCA(cams, fake0, fake1, fake2, start, end, num_pcs, thresh):
    ...
    for c in cams:
        ...
        camsOut.append(np.sum(c, axis = 0))
    ...
    fake0Out = np.sum(fake0, axis = 0)
    fake1Out = np.sum(fake1, axis = 0)
    fake2Out = np.sum(fake2, axis = 0)

    plt.title("cams plotted with sum sh coords, red is fake")
