...
def noPCA(cams, fake0, fake1, fake2, start, end, num_pcs, thresh):
    ...
    for c in cams:
        ...
        camsOut.append(np.linalg.norm(c[start:end, :], axis = 1))
...
def gen_results(i, fake_cams, num_cams, zero_start, data_dir, 
    ...
    plt.title("cams plotted in gen results, only SH coord 4, red is fake")
    plt.plot(cams[0][:,4], 'tab:blue')
    plt.plot(cams[1][:,4], 'tab:orange')
    plt.plot(cams[2][:,4], 'tab:green')
    plt.plot(cams[3][:,4], 'tab:purple')
    plt.plot(cams[4][:,4], 'tab:brown')
    plt.plot(cams[5][:,4], 'tab:pink')
    plt.plot(fake0[:,4], 'tab:red')
    plt.show()
