...
def gen_results(i, fake_cams, num_cams, zero_start, data_dir, 
    ...
    plt.title("cams plotted in gen results, only SH coord 1, red is fake")
    plt.plot(cams[0][:,1], 'tab:blue')
    plt.plot(cams[1][:,1], 'tab:orange')
    plt.plot(cams[2][:,1], 'tab:green')
    plt.plot(cams[3][:,1], 'tab:purple')
    plt.plot(cams[4][:,1], 'tab:brown')
    plt.plot(cams[5][:,1], 'tab:pink')
    plt.plot(fake0[:,1], 'tab:red')
    plt.show()
